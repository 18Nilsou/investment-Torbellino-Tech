...
2. Coordinates multiple exchanges and agentsine that:
3. Handles event processing and scheduling
4. Collects and records simulation resultsts
Handles event processing and scheduling
...
from market_sim.simulation.engine.blockchain_integration import BlockchainIntegration
...
class MarketSimulation:
    ...
    def schedule_event(self, timestamp: datetime, event_type: str, data: Any) -> None:
        ...
        event = SimulationEvent(timestamp, event_type, data)
          # Ensure heapq is imported at the top of the file
        heapq.heappush(self.event_queue, event)
    
    def initialize(self):
        """Initialize the simulation."""
        # Setup blockchain accounts for agents
        self.blockchain_integration.setup_ethereum_accounts(self.agents)
        
        # Deploy trade contracts for each symbol
        for symbol in self.exchanges.keys():
            self.blockchain_integration.deploy_trade_contract(symbol)
    ...
    def process_order(self, order: Order) -> List[Trade]:
        ...
        for trade in trades:
            ...
            self.blockchain_integration.record_trade_on_blockchain(trade)
        
        # Periodically mine blocks (e.g., every 10 trades)
        if len(self.blockchain_integration.blockchain.pending_transactions) >= 10:
            self.blockchain_integration.mine_block()
